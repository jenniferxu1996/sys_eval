...
import tqdm
from data_loader import SingleFrameDataloader
...
def inference(args):
    ...
    infer_data = SingleFrameDataloader(args, transform=transform)
    ...
    infer_loader = DataLoader(infer_data, 8, num_workers=4, shuffle=False)

    model = torch.load(f"ckpt_repo/{args.dataset}/{args.model}_{args.sampled_frame}.pt").to(device)
    print(f"from check point ckpt_repo/{args.dataset}/{args.model}_{args.sampled_frame}.pt")
